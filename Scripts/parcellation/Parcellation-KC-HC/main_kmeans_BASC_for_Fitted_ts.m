function main_kmeans_BASC_for_Fitted_ts(data_dir, result_dir, roiname,subject_list)
...
for subj = 1:length(subject_list)
    ...
    datafile = strcat(data_dir, subject{1}, '/Fitted_TS_Go.mat');
    xx = load(datafile); Xo = xx.avg_fitted_ts;
    %Coods = xx.vXYZ';
    Coods=[];
    xx = [];
