function [VI,NMI] = compare_clusterings( assig1, assig2 )

% Modified by Ryali to get both VI & NMI
...
VI = distance;
%NMI
H1 = -sum( p1.*locallog( p1 ))/log(2);
H2 = -sum( p2.*locallog( p2 ))/log(2);
I =  sum( sum( confusion.* locallog( cdummy )))/log(2);
NMI = (2*I)/(H1 + H2);
